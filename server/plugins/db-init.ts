...
export default defineNitroPlugin(async () => {
  ...
  `);
  await pool.query(`ALTER TABLE doc_versions ADD COLUMN IF NOT EXISTS action TEXT NOT NULL DEFAULT 'save'`);
  ...
  `);
  await pool.query(`CREATE INDEX IF NOT EXISTS release_versions_release_id_idx ON release_versions(release_id)`);
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS workspace_settings (
      ...
      description TEXT DEFAULT 'Documentation and changelog platform for product and engineering teams.',
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS internal_app_feedback (
      id TEXT PRIMARY KEY,
      user_id TEXT,
  ...
  await pool.query(`
    ALTER TABLE internal_app_feedback DROP CONSTRAINT IF EXISTS internal_app_feedback_app_id_apps_id_fk
  `);
  await pool.query(`
    ALTER TABLE internal_app_feedback DROP COLUMN IF EXISTS app_id
  `);
  ...
  if (wsCount[0]?.count === 0) {
    await db.insert(workspaceSettings).values({
      ...
      description: "Documentation and changelog platform for product and engineering teams.",
      theme: "light",
      logoUrl: null,
