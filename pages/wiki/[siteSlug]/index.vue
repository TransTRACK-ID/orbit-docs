...
const { fetchSite } = useInternalWiki();
...
async function load() {
  ...
  try {
    ...
    if (firstSlug) {
      router.replace(`${pathPrefix.value}/${firstSlug}`);
      return;
    }
    site.value = data;
  } catch (e: any) {
...
function firstPageSlug(
  navConfig: WikiSite["navConfig"],
  pages: Array<{ slug: string | null }>,
): string | null {
  const visibleSlugs = new Set(pages.map((p) => p.slug).filter(Boolean));
  const visitGroup = (g: { pages?: string[]; groups?: any[] }): string | null => {
    if (g.pages) {
      for (const s of g.pages) {
        if (visibleSlugs.has(s)) return s;
      }
    }
    if (g.groups) {
  ...
  const first = pages.find((p) => p.slug);
    ...
    <div class="doc-content-row">
      <article class="doc-body">
          ...
          <div v-for="group in apiGroups" :key="group.tag" style="margin-top: 20px">
            ...
            <ul class="ps-link-list">
              <li v-for="op in group.items" :key="op.slug">
                <NuxtLink :to="`${pathPrefix}/api/${op.slug}`" class="ps-link">
        ...
        <p v-if="!docPages.length && !apiGroups.length" class="ps-muted">
          This site has no pages or API operations yet.
        </p>
      </article>
    </div>
  </main>
